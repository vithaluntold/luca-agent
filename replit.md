...
11. **Critical Bug Fixes (November 10, 2025)**:
    ...
    - **Document Analyzer Agent**: Created dedicated document analyzer agent (server/services/agents/documentAnalyzer.ts) that wraps Azure Document Intelligence for thorough document scanning. Extracts structured data, key findings, tables, and key-value pairs with fallback to pdf-parse for text extraction. Ready for integration into multi-agent orchestration layer.
