...
The backend uses Express.js with a PostgreSQL database and Drizzle ORM. Security features include bcrypt for password hashing (minimum 6 characters), AES-256-GCM encryption for sensitive data, Helmet middleware, and rate limiting with trust proxy enabled. File uploads utilize a secure system with per-file encryption, key wrapping, SHA-256 checksums, and virus scanning. The `conversations` table includes a `profileId` column for efficient profile-based filtering, and the system enforces profile ownership and constraints. Authentication flow uses a two-step redirect pattern (shouldRedirect flag + useEffect) to prevent race conditions with user state propagation.
