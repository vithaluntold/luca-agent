...
The Output Pane features a comprehensive export system that generates professional-grade documents in six formats: TXT, CSV, DOCX, PDF, PPTX, and XLSX. The DocumentExporter service on the backend uses specialized libraries (docx, pdfkit, pptxgenjs, exceljs) to create properly formatted files from markdown content. Export controls are conditionally enabled only when curated content (calculations, documents, visualizations) with `metadata.showInOutputPane=true` is present, preventing errors from empty exports. The system includes search functionality, copy-to-clipboard, and formatted/code view modes for the output content.
